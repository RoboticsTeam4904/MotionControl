...
strictfp public class MotionTrajectoryExecutor {
	...
	public static void main(String[] args) {
		...
		System.out.println("##############################");
		MotionTrajectory motionTrajectory = new MotionTrajectory(spline, 0.5, 10, 200);
		...
		for(int i = 0; i < motionTrajectory.featureSegments.size() - 1; i++) {
			System.out.println("RIGHT{" + i + "}" + motionTrajectory.rightWheel.trajectorySegments.get(i).duration + " LEFT{" + i + "}" + motionTrajectory.leftWheel.trajectorySegments.get(i).duration + " DIF # " + (motionTrajectory.rightWheel.trajectorySegments.get(i).duration - motionTrajectory.leftWheel.trajectorySegments.get(i).duration));
		...
		Tuple<MotionTrajectoryPoint, MotionTrajectoryPoint> lastPoint = new Tuple<>(motionTrajectoryPoint, motionTrajectoryPoint);
		for(int i = 1; i < 200; i++) {
